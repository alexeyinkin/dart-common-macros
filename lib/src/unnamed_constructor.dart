...
macro class UnnamedConstructor implements ClassDeclarationsMacro {
  ...
  Future<void> buildDeclarationsForClass(
    ...
    await _assertNoUnnamedConstructor(clazz, builder);

    final superCtor = await _requireUnnamedSuperConstructor(clazz, builder);
    final thisParams = await _getThisParams(clazz, builder);
  ...
  Future<void> _assertNoUnnamedConstructor(
    ClassDeclaration clazz,
    MemberDeclarationBuilder builder,
  ) async {
    final constructors = await builder.constructorsOf(clazz);
    final unnamedConstructor =
        constructors.firstWhereOrNull((c) => c.identifier.name == '');
